...
class Nice_Trainer():
    def __init__(self, config):
        ...
        self.dataloader = DataLoader(ImageDataset(config['dataroot'], noise_level=level,transforms_1=transforms_1, transforms_2=transforms_2, unaligned=False),
    ...
    def train(self):
        ...
        self.netG_A2B.load_state_dict(torch.load('./output/Nice/NC+R/netG_A2B.pth'))
        self.netG_A2B.apply(weights_init_normal)
        # source = next(iter(self.dataloader))['A']
        # print(np.transpose(source[0].numpy()).shape)
        # cv2.imwrite('./ex1.jpg',np.transpose(source[0].numpy())*255)
    ...
    #         # Save models checkpoints
            ...
            with torch.no_grad():
                ...
                for i, batch in enumerate(self.val_data):
                    real_A = Variable(self.input_A.copy_(batch['A']))
                    real_B = Variable(self.input_B.copy_(batch['B'])).detach().cpu().numpy().squeeze()[0]
                    # print('realB',real_B.shape)
                    if self.config['bidirect']:
                        _,  _,  _, _, real_A_z = self.netD_A(real_A)
                        fake_B = self.netG_A2B(real_A_z).detach().cpu().numpy().squeeze()[0]
                        # print(fake_B.shape)
                    else:
                        fake_B = self.netG_A2B(real_A).detach().cpu().numpy().squeeze()[0]
                        # print(fake_B.shape)
    ...
    def test(self):
        ...
        self.netG_A2B.load_state_dict(torch.load(r'C:\Users\NM_RR\Desktop\SJ\Reg-GAN-main\output\Nice\NC+R\retrained\netG_A2B.pth'))
        with torch.no_grad():
                MAE = 0
                PSNR = 0
                SSIM_1 = 0
                ...
                for i, batch in enumerate(self.val_data):
                    ...
                    name_B = batch['name_B']
                    real_A = Variable(self.input_A.copy_(batch['A']))
                    real_B_s = Variable(self.input_B.copy_(batch['B']))
                    fake_B_s = self.netG_A2B(real_A)
                    ...
                    cv2.imwrite('C:/Users/NM_RR/Desktop/SJ/Reg-GAN-main/output/visualized/origin/'+name_A[0],real_A*255)
                    cv2.imwrite('C:/Users/NM_RR/Desktop/SJ/Reg-GAN-main/output/visualized/fake_B/'+name_B[0],fake_B*255)
                    cv2.imwrite('C:/Users/NM_RR/Desktop/SJ/Reg-GAN-main/output/visualized/real_B/'+name_B[0] ,real_B*255)
                    num += 1
