...
class Palette(BaseModel):
    ...
    def test(self):
        ...
        with torch.no_grad():
            for i, phase_data in tqdm.tqdm(enumerate(self.phase_loader)):
                ...
                if self.opt['distributed']:
                    ...
                    if self.task in ['inpainting','uncropping']:
                        ...
                        self.output, self.visuals = self.netG.restoration(self.cond_image, sample_num=self.sample_num)
                fake = self.output.cpu().numpy().squeeze() 
                cv2.imwrite('C:/Users/NM_RR/Desktop/SJ/Palette-Image-to-Image-Diffusion-Models-main/experiments/visualized/fake/'+str(i)+'.jpg',fake*255)       
                # self.iter += self.batch_size
